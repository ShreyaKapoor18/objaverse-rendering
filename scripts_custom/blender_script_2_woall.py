...
# Use glob.glob() to find files matching the patterns
list_env_maps = glob.glob(env_maps_dir)
list_textures_dir = glob.glob(textures_dir)
...
def download_object(object_url: str) -> str:
    ...
    tmp_local_path = os.path.join("tmp-objects-2", f"{uid}.glb" + ".tmp")
    local_path = os.path.join("tmp-objects-2", f"{uid}.glb")
    # wget the file and put it in local_path
    if not os.path.exists('tmp-objects-2'):
        if not os.path.exists(local_path):
            os.makedirs(os.path.dirname(tmp_local_path))
