...
def add_environment_map(list_env_maps, i):
    ...
    node_env.image = bpy.data.images.load(list_env_maps[i])  # Replace "/path/to/your/hdri/image.hdr" with the path to your HDRI image

    # Disconnect the Environment Texture node from the shader tree
    links = tree.links
    for link in links:
        if link.to_node == node_env:
            links.remove(link)

    # Create a new Background node to maintain scene lighting
    node_bg = nodes.new(type='ShaderNodeBackground')
    link_bg = links.new(node_bg.outputs[0], tree.nodes['World Output'].inputs['Surface'])
...
def add_background_image(list_env_maps, i):
    ...
    for obj in bpy.context.selected_objects:
        ...
        radius_multiplier = 100  # Adjust the multiplier as needed
        bpy.ops.mesh.primitive_hemisphere_add(radius=obj.dimensions.length * radius_multiplier, location=bounding_box_center)
        hemisphere = bpy.context.object
     
        # Add hemisphere at bounding box center
        bpy.ops.mesh.primitive_hemisphere_add(radius=1, location=bounding_box_center)
        hemisphere = bpy.context.object
        
        # Adjust hemisphere properties as needed
        ...
        emission_node.inputs['Strength'].default_value = 1.0  # Adjust strength as needed
