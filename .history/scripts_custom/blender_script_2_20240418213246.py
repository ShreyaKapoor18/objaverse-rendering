...
def add_background_image():
    ...
    env_tex_node = bpy.context.scene.world.node_tree.nodes.new(type='ShaderNodeTexEnvironment')

    # Set the image file path for the environment texture
    env_tex_node.image = bpy.data.images.load("path_to_your_image.jpg")

    # Add Ambient Color node
    ambient_color_node = bpy.context.scene.world.node_tree.nodes.new(type='ShaderNodeRGB')
    ambient_color_node.outputs[0].default_value = (0.2, 0.2, 0.2, 1.0)  # Set ambient color with alpha value (adjust as needed)

    # Add Mix Shader node
    mix_shader_node = bpy.context.scene.world.node_tree.nodes.new(type='ShaderNodeMixShader')
    mix_shader_node.inputs[0].default_value = 1.0  # Set factor to control the influence of ambient color

    # Link nodes
    bpy.context.scene.world.node_tree.links.new(ambient_color_node.outputs[0], mix_shader_node.inputs[1])
    bpy.context.scene.world.node_tree.links.new(env_tex_node.outputs[0], mix_shader_node.inputs[2])

    # Set the world output node
    output_node = bpy.context.scene.world.node_tree.nodes.new(type='ShaderNodeOutputWorld')
    bpy.context.scene.world.node_tree.links.new(mix_shader_node.outputs[0], output_node.inputs[0])
