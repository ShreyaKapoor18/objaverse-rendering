...
def add_background_image(list_env_maps, i):
    ...
    # Clear all nodes in the material
    ...
    # Create Glossy Depth input
    glossy_depth = nodes.new("ShaderNodeLightPath")
    glossy_depth.location = (-300, 300)  # Example location

    # Create Environment Texture node
    env_tex = nodes.new("ShaderNodeTexEnvironment")
    env_tex.image = bpy.data.images.load(list_env_maps[i])  # Set the path to your .exr file
    env_tex.location = (0, 300)  # Example location

    # Create Mix Shader node
    mix_shader = nodes.new("ShaderNodeMixShader")
    mix_shader.location = (300, 300)  # Example location

    # Create Background node
    background = nodes.new("ShaderNodeBackground")
    background.location = (600, 300)  # Example location
    background.inputs["Strength"].default_value = 2.0  # Set the strength to 2.00 (or adjust as needed)

    # Create World Output node
    world_output = nodes.new("ShaderNodeOutputWorld")
    world_output.location = (900, 300)  # Example location

    # Connect nodes
    links.new(glossy_depth.outputs["Glossy Depth"], mix_shader.inputs[0])
    links.new(env_tex.outputs["Color"], mix_shader.inputs[1])
    links.new(mix_shader.outputs["Shader"], background.inputs["Color"])
    links.new(background.outputs["Background"], world_output.inputs["Surface"])

    # Assign the material to the object(s)
...
def add_background_map(list_env_maps, i):
    ...
    filepath = list_env_maps[i]  # Get the file path of the new image
    img = bpy.data.images.load(filepath)  # Load the new image
