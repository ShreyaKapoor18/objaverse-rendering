...
def add_background_image(list_env_maps, i):
    ...
    #emission_node.location = 0, 0  # Position the node in the editor

    # Set emission color and strength for uniform lighting
    emission_node.inputs[0].default_value = (1.0, 1.0, 1.0, 1.0)  # RGB color (white)
    emission_node.inputs[1].default_value = 5.0  # Strength

    # Connect Emission node to the Surface input of the World Output node
    world_output_node = tree.nodes.get('World Output')
    #world_output_node.location = 400, 0  # Move the World Output node to make space
    link = tree.links.new(emission_node.outputs[0], world_output_node.inputs[0])
