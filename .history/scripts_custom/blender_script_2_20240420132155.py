...
def calculate_bounding_box_center(obj):
    bounding_box = obj.bound_box
    min_corner = mathutils.Vector((bounding_box[0][0], bounding_box[0][1], bounding_box[0][2]))
    max_corner = mathutils.Vector((bounding_box[6][0], bounding_box[6][1], bounding_box[6][2]))
...
for obj in bpy.context.selected_objects:
    ...
    bounding_box_center = calculate_bounding_box_center(obj)
    
    # Add hemisphere lights around the bounding box
    radius_multiplier = 10  # Adjust the multiplier as needed
    coordinates = [(np.random.uniform(-1, 1, size=1)) for _ in range(5)]
    pairs = list(itertools.product(coordinates, repeat=3))
    for pair in pairs:
        #print(pair)
        #print(bounding_box_center)
        #print(pair)
        bpy.ops.mesh.primitive_uv_sphere_add(radius=radius_multiplier, location=(pair[0], pair[1], pair[2]))  
        hemisphere = bpy.context.object
        # Adjust hemisphere properties
        hemisphere_material = bpy.data.materials.new(name="AmbientLightMaterial")
        hemisphere_material.use_nodes = True
        node_tree = hemisphere_material.node_tree
        emission_node = node_tree.nodes.new('ShaderNodeEmission')
        emission_strength = 100  # Adjust strength as needed
        emission_node.inputs['Strength'].default_value = emission_strength
        node_tree.links.new(emission_node.outputs['Emission'], node_tree.nodes['Material Output'].inputs['Surface'])
        hemisphere.data.materials.append(hemisphere_material)
        ...
        light.energy = 1000  # Adjust the brightness as needed
        ...
        # Move the ground plane down
        bpy.context.object.location.z = -1

        # Add a sphere to emit light
        bpy.ops.mesh.primitive_uv_sphere_add(radius=1, location=(0, 0, 2))  # Adjust location as needed

        # Access the newly added sphere object
        sphere_obj = bpy.context.object

        # Add emission shader to the sphere
        ...
        emission.inputs[0].default_value = (1, 1, 1, 1)  # White emission color
        emission.inputs[1].default_value = 100  # Strength of emission
        ...
        # Set up the camera
        bpy.ops.object.camera_add(location=(0, -5, 2), rotation=(1.0472, 0, 0))  # Location and rotation values are in radians
        bpy.context.scene.camera = bpy.context.object

        # Set camera to perspective mode
        bpy.data.cameras[bpy.context.object.data.name].type = 'PERSP'

        # Set render settings
        bpy.context.scene.render.engine = 'CYCLES'
        bpy.context.scene.cycles.samples = 100  # Increase samples for better quality

        # Set world background color (optional)
        bpy.context.scene.world.use_nodes = False  # Disable world nodes
        bpy.context.scene.world.color = (0.1, 0.1, 0.1)  # Dark gray background
        ...
        #hemisphere.data.shadow_method = 'RAY_SHADOW'
        #hemisphere.data.cycles_visibility.shadow = True
