...
def add_background_image(list_env_maps, i):
    ...
    world.use_nodes = True
    ambient_node = world.node_tree.nodes.new(type='ShaderNodeBackground')
    ambient_node.inputs['Strength'].default_value = ambient_light_strength

    # Disconnect any existing nodes
    for link in world.node_tree.links:
        world.node_tree.links.remove(link)

    # Connect the ambient node to the output
    world.node_tree.links.new(ambient_node.outputs['Background'], world.node_tree.nodes['World Output'].inputs['Surface'])
