...
def add_background(list_env_maps, i):
    ...
    # Check if the environment texture node already exists
    env_node = None
    for node in nodes:
    ...
    if env_node is None:
        # Create a new environment texture node if it doesn't exist
        env_node = nodes.new("ShaderNodeTexEnvironment")
        env_node.location = (-300, 300)  # Set the node location
    else:
        # Clear existing connections
        for link in env_node.outputs['Color'].links:
            links.remove(link)
    ...
    # Create the output world node if it doesn't exist
    output_node = nodes.get('ShaderNodeOutputWorld')
    if output_node is None:
        output_node = nodes.new('ShaderNodeOutputWorld')
        output_node.location = (400, 0)  # Set the node location
    
    # Connect the environment texture node to the output world node
    links.new(env_node.outputs['Color'], output_node.inputs['Surface'])
...
def save_images(object_file: str) -> None:
    ...
    for i in range(args.num_images):
        ...
        add_background(list_env_maps, i)
