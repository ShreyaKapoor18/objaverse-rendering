...
def add_background_image(list_env_maps, i):
    ...
    for obj in bpy.context.selected_objects:
        ...
        import itertools
        #coordinates = [-1, 0,1]
        coordinates = [tuple(np.random.uniform(-1,1, size=10)) for _ in range(10)]

        pairs = list(itertools.product(coordinates, repeat=3))
        for pair in pairs:
            print(pair)
            result = tuple(x * y for x, y in zip(pair, bounding_box_center.x, bounding_box_center.y, bounding_box_center.z))
            
            bpy.ops.mesh.primitive_uv_sphere_add(radius=radius_multiplier, location=result)  # Upper hemisphere sphere
