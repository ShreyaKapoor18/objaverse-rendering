...
def add_background_image(list_env_maps, i):
    ...
    for obj in bpy.context.scene.objects:
        ...
        print('The bounding box corners are', corners)
        for obj in bpy.data.objects:
            if obj.type == 'LIGHT' and obj.data.type == 'POINT':
                # Create a new emission node
                emission_node = world.node_tree.nodes.new('ShaderNodeEmission')
                
                # Connect the emission node to the world output node
                world_output = world.node_tree.nodes.get('World Output')
                world.node_tree.links.new(emission_node.outputs[0], world_output.inputs['Surface'])
                
                # Connect the light strength to the emission node
                emission_node.inputs['Strength'].default_value = obj.data.energy
