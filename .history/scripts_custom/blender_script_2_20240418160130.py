...
def add_background(list_env_maps, i):
    ...
    if nodes.find('Environment Texture') == -1:
        print("Env node not found")
        envNode = nodes.new("ShaderNodeTexEnvironment")
    else:
        print("Environment texture is not found")
    ...
    envNode.location.x = world_output_node.location.x-300
    envNode.location.y = world_output_node.location.y
    links.new(envNode.outputs['Color'], world_output_node.inputs['Surface'])
    world.node_tree.links.new(envNode.outputs['Color'], world_output_node.inputs['Surface'])
