...
def add_background_image(list_env_maps, i):
    ...
    # Set emission color and strength for the mesh to illuminate itself
    emission_node.inputs[0].default_value = (1.0, 1.0, 1.0, 1.0)  # Color
    ...
    # Link emission node to material output node
    emission_link = emission_mat.node_tree.links.new(emission_node.outputs[0], output_node.inputs[0])
    bpy.ops.object.select_all(action='SELECT')
    bpy.ops.object.select_all(action='DESELECT')
