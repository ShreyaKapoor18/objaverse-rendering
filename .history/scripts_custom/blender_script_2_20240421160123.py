...
def add_uv_sphere(location, radius):
...
def add_background_image(list_env_maps, i):
    ...
    # Select all objects in the current sce
    #bpy.context.scene.world.light_settings.ao_distance = 10.0
    # Set ambient occlusion factor (strength)
    # Function to add a Sun lamp
    def add_sun_lamp(location, strength):
        lamp = bpy.data.lights.new(name="SunLamp", type='SUN')
        lamp.energy = strength
        
        # Create lamp object
        lamp_obj = bpy.data.objects.new(name="SunLampObject", object_data=lamp)
        bpy.context.collection.objects.link(lamp_obj)
        lamp_obj.location = location
        
        # Set lamp direction (default: pointing in -Z direction)
        lamp_obj.rotation_euler = (0, 0, 0)  # Adjust as needed
    ...
    for obj in selected_objects:
        ...
        # Add a Sun lamp at the location of each selected object with a strength of 10.0
        for corner in corners:
            add_sun_lamp(corner, 20.0)
            
    #scene.render.use_ambient_occlusion = False
    
    # Remove other occlusion-related settings as needed
    # For example:
    ...
    scene.render.use_subsurface = False
