...
def add_area_light(intensity):
    # Create a new area light
    light_data = bpy.data.lights.new(name="Ambient Light", type='AREA')
    light_data.energy = intensity  # Set light intensity
    ...
    # Place the light object in the scene
    light_object.location = (0, 0, 5)  # Adjust the location as needed
...
def add_background_image(list_env_maps, i):
    ...
    if backNode:
    # Set the strength of the background shader to 0
        backNode.inputs['Strength'].default_value = 0.0
    bpy.context.scene.world.light_settings.use_ambient_occlusion = True
    #bpy.context.scene.world.light_settings.environment_color = 'PLAIN'
    
    emission_node = world.node_tree.nodes.new(type='ShaderNodeEmission')

    # Set the color and strength of the emission shader
    emission_node.inputs['Color'].default_value = (1.0, 1.0, 1.0, 1.0)  # Adjust the color as needed
    emission_node.inputs['Strength'].default_value = 0.5  # Adjust the strength as needed

    # Create a Mix shader node to combine the emission and background shaders
    mix_node = world.node_tree.nodes.new(type='ShaderNodeMixShader')

    # Set the factor to control the mix between emission and background
    mix_node.inputs['Fac'].default_value = 0.5  # Adjust the factor as needed

    # Access the existing background shader node
    background_node = world.node_tree.nodes.get('Background')

    # Connect the emission shader to the first input of the Mix shader
    world.node_tree.links.new(emission_node.outputs['Emission'], mix_node.inputs[1])

    # Connect the background shader to the second input of the Mix shader
    world.node_tree.links.new(background_node.outputs['Background'], mix_node.inputs[2])

    # Set the output of the Mix shader as the world's surface
    world.node_tree.links.new(mix_node.outputs['Shader'], world.node_tree.nodes['World Output'].inputs['Surface'])
...
def save_images(object_file: str) -> None:
    ...
    for i in range(args.num_images):
        ...
        #add_background_image(list_env_maps,i, intensity=0.5)
