...
# Parameters for light setup
    num_lights = 4  # Number of lights to add
    light_distance = 5  # Distance between lights

    # Add area lights
    for i in range(num_lights):
        bpy.ops.object.light_add(type='AREA', radius=1, align='WORLD', location=(0, 0, 0))
        area_light = bpy.context.object
        area_light.location = (0, 0, i * light_distance)  # Place lights along Z-axis

    # Add point lights
    for i in range(num_lights):
        bpy.ops.object.light_add(type='POINT', radius=1, align='WORLD', location=(0, 0, 0))
        point_light = bpy.context.object
        point_light.location = (i * light_distance, 0, 0)  # Place lights along X-axis

    # Get scene bounding box dimensions
    bbox = [obj for obj in bpy.context.scene.objects if obj.type == 'MESH']
    min_x = min(obj.location.x + min(v.x for v in obj.dimensions) for obj in bbox)
    min_y = min(obj.location.y + min(v.y for v in obj.dimensions) for obj in bbox)
    min_z = min(obj.location.z + min(v.z for v in obj.dimensions) for obj in bbox)
    max_x = max(obj.location.x + max(v.x for v in obj.dimensions) for obj in bbox)
    max_y = max(obj.location.y + max(v.y for v in obj.dimensions) for obj in bbox)
    max_z = max(obj.location.z + max(v.z for v in obj.dimensions) for obj in bbox)
    ...
    # Distribute area lights evenly around the scene
    for i, area_light in enumerate(bpy.data.objects[:num_lights]):
        angle = (i / num_lights) * 2 * math.pi
        x = center_x + (max_x - min_x) * 0.5 * math.cos(angle)
        y = center_y + (max_y - min_y) * 0.5 * math.sin(angle)
        area_light.location = (x, y, area_light.location.z)

    # Distribute point lights evenly along X-axis
    for i, point_light in enumerate(bpy.data.objects[num_lights:]):
        point_light.location = (i * light_distance, 0, point_light.location.z)
