...
def add_background_image(list_env_maps, i):
    ...
    emission_mat = bpy.data.materials.new(name="Self Illuminating")
    emission_mat.use_nodes = True

    # Clear existing nodes
    for node in emission_mat.node_tree.nodes:
        emission_mat.node_tree.nodes.remove(node)

    # Add Texture Coordinate node
    tex_coord_node = emission_mat.node_tree.nodes.new(type='ShaderNodeTexCoord')

    # Add Mapping node to adjust texture coordinates if necessary
    mapping_node = emission_mat.node_tree.nodes.new(type='ShaderNodeMapping')

    # Add Image Texture node to sample the color texture of the mesh
    image_texture_node = emission_mat.node_tree.nodes.new(type='ShaderNodeTexImage')
    # Assign the color texture of the mesh to the Image Texture node
    for obj in bpy.context.scene.objects:
        if obj.type == 'MESH':
            object = obj
            image_texture_node.image = object.active_material.node_tree.nodes['Image Texture'].image  # Replace 'Cube' with the name of your object

    # Add Color Ramp node
    color_ramp_node = emission_mat.node_tree.nodes.new(type='ShaderNodeValToRGB')

    # Add emission node
    emission_node = emission_mat.node_tree.nodes.new(type='ShaderNodeEmission')

    # Add Principled BSDF node
    principled_bsdf_node = emission_mat.node_tree.nodes.new(type='ShaderNodeBsdfPrincipled')

    # Add Material Output node
    output_node = emission_mat.node_tree.nodes.new(type='ShaderNodeOutputMaterial')

    # Connect Texture Coordinate node to Mapping node
    emission_mat.node_tree.links.new(tex_coord_node.outputs[2], mapping_node.inputs[0])  # Use UV coordinates

    # Connect Mapping node to Image Texture node
    emission_mat.node_tree.links.new(mapping_node.outputs[0], image_texture_node.inputs[0])

    # Connect Image Texture node to Color Ramp node
    emission_mat.node_tree.links.new(image_texture_node.outputs[0], color_ramp_node.inputs[0])

    # Connect Color Ramp node to Emission node
    emission_mat.node_tree.links.new(color_ramp_node.outputs[0], emission_node.inputs[0])

    # Connect Color Ramp node to Base Color input of Principled BSDF node
    emission_mat.node_tree.links.new(color_ramp_node.outputs[0], principled_bsdf_node.inputs['Base Color'])

    # Connect Principled BSDF node to Material Output node
    emission_mat.node_tree.links.new(principled_bsdf_node.outputs[0], output_node.inputs[0])
    mesh_objects = [obj for obj in bpy.context.scene.objects if obj.type == 'MESH']

    for obj in mesh_objects:
    # Iterate over each material of the mesh object
        for slot in obj.material_slots:
            material = slot.material
            if material:
                image_texture_node.image = material.node_tree.nodes.get('Image Texture').image
