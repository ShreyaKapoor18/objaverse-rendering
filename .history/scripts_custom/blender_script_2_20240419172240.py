...
def add_background_image(list_env_maps, i):
    ...
    node.image = bpy.data.images.load(list_env_maps[i])  # Replace "/path/to/your/hdri/image.hdr" with the path to your HDRI image
    world_output = nodes.get('World Output')
    links = tree.links
    link = links.new(node.outputs[0], world_output.inputs[0])
    ...
    image_tex_node = nodes.new(type='ShaderNodeTexEnvironment')
    background_node = nodes.new(type='ShaderNodeBackground')
    emission_node = nodes.new(type='ShaderNodeEmission')
    mix_shader_node = nodes.new(type='ShaderNodeMixShader')
    output_node = nodes.new(type='ShaderNodeOutputMaterial')

    # Set color for emission shader
    emission_node.inputs['Color'].default_value = (1, 1, 1, 1)  # White color

    # Connect nodes
    mat.node_tree.links.new(tex_coord_node.outputs['Generated'], mapping_node.inputs['Vector'])
    mat.node_tree.links.new(mapping_node.outputs['Vector'], image_tex_node.inputs['Vector'])
    mat.node_tree.links.new(image_tex_node.outputs['Color'], mix_shader_node.inputs[2])
    mat.node_tree.links.new(emission_node.outputs['Emission'], mix_shader_node.inputs[1])
    mat.node_tree.links.new(mix_shader_node.outputs['Shader'], output_node.inputs['Surface'])

    # Adjust sphere settings
