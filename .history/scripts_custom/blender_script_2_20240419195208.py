...
def add_background_image(list_env_maps, i):
    ...
    links.new(envNodeColorOut, backColIn)
    if envNode.image:
        for pixel_index, pixel_value in enumerate(envNode.image.pixels):
            envNode.image.pixels[pixel_index] = pixel_value * 0.5
    bpy.context.scene.render.film_transparent = False
    envNode.image = bpy.data.images.load(image_path)
    envNode.image = bpy.data.images.load(image_path)
    ...
    for obj in bpy.context.selected_objects:
        ...
        emission_node.inputs['Strength'].default_value = 0.5  # Adjust strength as needed
