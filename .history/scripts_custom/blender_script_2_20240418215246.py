...
def add_background_image(list_env_maps, i):
    ...
    material = bpy.data.materials.new(name="Background Material")
    ...
    env_texture = bpy.data.textures.new("Environment Texture", type='IMAGE')
    env_texture.image = bpy.data.images.load(list_env_maps[i])

    # Add environment lighting
    #bpy.context.scene.world.light_settings.use_environment_light = True

    # Adjust the ambient lighting intensity if needed
    #bpy.context.scene.world.light_settings.environment_energy = 1.0

    # Assign the environment texture to the plane material
    #plane_obj.data.materials[0].texture_slots.add()
    #plane_obj.data.materials[0].texture_slots[0].texture = env_texture

    # Set mapping coordinates
    #plane_obj.data.materials[0].texture_slots[0].texture_coords = 'UV'

    # Set the mapping to Environment
    #plane_obj.data.materials[0].texture_slots[0].mapping = 'REFLECTION'

    # Set the blend type to Mix
    #plane_obj.data.materials[0].texture_slots[0].blend_type = 'MIX'
    
def background_image(list_env_maps, i):
    ...
    backNode = nodes.get('Background')

    # Check if the Environment Texture node already exists, if not, create one
    if nodes.get('Environment Texture') is None:
        envNode = nodes.new("ShaderNodeTexEnvironment")
    else:
        envNode = nodes.get('Environment Texture')

    # Position the Environment Texture node
    envNode.location.x = backNode.location.x - 300 if backNode else 0
    envNode.location.y = backNode.location.y if backNode else 0
    ...
    # Load the image as the environment map
