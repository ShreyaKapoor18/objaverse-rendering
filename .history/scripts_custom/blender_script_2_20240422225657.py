...
def calculate_bbox_dimensions(objects):
    min_x = min(obj.location.x + min(v.x for v in obj.dimensions) for obj in objects)
    min_y = min(obj.location.y + min(v.y for v in obj.dimensions) for obj in objects)
    min_z = min(obj.location.z + min(v.z for v in obj.dimensions) for obj in objects)
    max_x = max(obj.location.x + max(v.x for v in obj.dimensions) for obj in objects)
    max_y = max(obj.location.y + max(v.y for v in obj.dimensions) for obj in objects)
    max_z = max(obj.location.z + max(v.z for v in obj.dimensions) for obj in objects)
...
def add_background_image(list_env_maps, i):
    ...
    # Create a new plane
    plane_size = max(max_corner) - min(min_corner)
    bpy.ops.mesh.primitive_plane_add(size=plane_size)  # Adjust the size as needed
    ...
    # Position the plane outside the bounding box of the scene
    plane_location = [(min_coord + max_coord) / 2 for min_coord, max_coord in zip(min_corner, max_corner)]
    plane_location[2] = max_corner[2] + plane_size / 2  # Position above the bounding box
