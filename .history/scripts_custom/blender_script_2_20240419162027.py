...
def add_background_image(list_env_maps, i):
    lighting_env_map_path = list_env_maps[i]
    lighting_env_tex = bpy.data.images.load(lighting_env_map_path)
    bpy.context.scene.world.node_tree.nodes['Environment Texture'].image = lighting_env_tex

    # Set up background environment map
    background_env_map_path = "/path/to/background_env_map.exr"
    background_env_tex = bpy.data.images.load(background_env_map_path)
    bg_env_node = bpy.context.scene.world.node_tree.nodes.new("ShaderNodeTexEnvironment")
    bg_env_node.image = background_env_tex
    bpy.context.scene.world.node_tree.nodes['Background'].inputs['Color'].default_value = (0, 0, 0, 1)  # Set background to black
    bpy.context.scene.world.node_tree.links.new(bg_env_node.outputs['Color'], bpy.context.scene.world.node_tree.nodes['Background'].inputs['Color'])
