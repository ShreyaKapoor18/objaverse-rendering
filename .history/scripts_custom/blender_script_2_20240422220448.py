...
def add_background_image(list_env_maps, i):
    ...
    # Create a new material for each object
        ...
        if active_material:
            # Iterate over each node in the active material's node tree
            for node in active_material.node_tree.nodes:
                # Check if the node is an Image Texture node
                if node.type == 'TEX_IMAGE':
                    # Create Image Texture node in the emission material's node tree
                    image_texture_node = emission_mat.node_tree.nodes.new(type='ShaderNodeTexImage')

                    # Set the image texture to the same image as the active material
                    ...
                    # Exit the loop after finding the first Image Texture node
                    break
