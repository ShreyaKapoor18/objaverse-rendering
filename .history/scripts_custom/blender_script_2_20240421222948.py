...
def add_background_image(list_env_maps, i):
    ...
    emission_node.inputs[0].default_value = (1.0, 1.0, 1.0, 1.0)  # RGB color (white)
    emission_node.inputs[1].default_value = 5.0  # Strength

    # Connect Emission node to the Material Output node
    output_node = emission_mat.node_tree.nodes.get('Material Output')
    link = emission_mat.node_tree.links.new(emission_node.outputs[0], output_node.inputs[0])
