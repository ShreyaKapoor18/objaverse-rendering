...
def add_background_image(list_env_maps, i):
    ...
    # Create or retrieve the worls
    image_path = list_env_maps[i]
     # Adjust this value to change intensity (0.5 reduces intensity by half)
    bpy.context.scene.world.use_nodes = True
    tree = bpy.context.scene.world.node_tree
    nodes = tree.nodes
    node_env = nodes.new(type='ShaderNodeTexEnvironment')
    node_env.image = bpy.data.images.load("/path/to/your/hdri/image.hdr")  # Replace "/path/to/your/hdri/image.hdr" with the path to your HDRI image

    # Create a Mix node to control influence
    node_mix = nodes.new(type='ShaderNodeMixShader')
    node_mix.inputs[0].default_value = 1.0  # Full influence of environment texture (for reflections)
    node_mix.location = 300, 0  # Optional: Adjust node position for clarity

    # Disconnect the Background node
    links = tree.links
    for link in links:
        if link.to_node == nodes['Background']:
            links.remove(link)

    # Link nodes
    link_env = links.new(node_env.outputs['Color'], node_mix.inputs[2])

    # Create a new Background node to maintain scene lighting
    node_bg = nodes.new(type='ShaderNodeBackground')
    link_bg = links.new(node_mix.outputs[0], node_bg.inputs[0])
    bpy.context.scene.render.film_transparent = False
    node_env.image = bpy.data.images.load(image_path)
    # Add ambient light
    bpy.ops.object.light_add(type='AREA', location=(0, 0, 10))
    light = bpy.context.object
    light.data.energy = 1.0  # Adjust the energy of the light source as needed
    light.data.color = (1, 1, 1)  # Adjust the color of the light source as needed
