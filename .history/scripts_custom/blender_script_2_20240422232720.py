...
def add_background_image(list_env_maps, i):
    ...
    bpy.context.scene.world.cycles_visibility.transmission = False
    bpy.context.scene.world.cycles_visibility.scatter = False
    bpy.context.scene.render.bake.ao.use_bake_ao = False


    # Get the active object (assuming it's a mesh object)
    mesh_objects = [obj for obj in bpy.context.scene.objects if obj.type == 'MESH']
    # Calculate the dimensions of the scene's bounding box
    min_corner, max_corner = scene_bbox()

    # Calculate the dimensions and position of each plane
    bbox_width = max_corner[0] - min_corner[0]
    bbox_height = max_corner[1] - min_corner[1]
    bbox_depth = max_corner[2] - min_corner[2]

    # Create a plane on top
    bpy.ops.mesh.primitive_plane_add(size=bbox_width)
    plane_top = bpy.context.object
    plane_top.location = ((min_corner[0] + max_corner[0]) / 2, (min_corner[1] + max_corner[1]) / 2, max_corner[2] + bbox_depth / 2)
    plane_top.hide_render = True

    # Create a plane on bottom
    bpy.ops.mesh.primitive_plane_add(size=bbox_width)
    plane_bottom = bpy.context.object
    plane_bottom.location = ((min_corner[0] + max_corner[0]) / 2, (min_corner[1] + max_corner[1]) / 2, min_corner[2] - bbox_depth / 2)
    plane_bottom.hide_render = True

    # Create a plane on left
    bpy.ops.mesh.primitive_plane_add(size=bbox_height)
    plane_left = bpy.context.object
    plane_left.location = (min_corner[0] - bbox_width / 2, (min_corner[1] + max_corner[1]) / 2, (min_corner[2] + max_corner[2]) / 2)
    plane_left.hide_render = True

    # Create a plane on right
    bpy.ops.mesh.primitive_plane_add(size=bbox_height)
    plane_right = bpy.context.object
    plane_right.location = (max_corner[0] + bbox_width / 2, (min_corner[1] + max_corner[1]) / 2, (min_corner[2] + max_corner[2]) / 2)
    plane_right.hide_render = True

    # Create a new material for the planes
    mat = bpy.data.materials.new(name="Ambient Light Material")
    mat.use_nodes = True
    ...
    emission_node.inputs[0].default_value = (1, 1, 1, 1)  # Set emission color to white
    emission_node.inputs[1].default_value = 100.0  # Set emission strength

    # Create Transparent BSDF shader
    ...
    links.new(emission_node.outputs[0], mix_shader_node.inputs[1])  # Connect emission shader to Mix Shader
    links.new(transparent_node.outputs[0], mix_shader_node.inputs[2])  # Connect transparent shader to Mix Shader
    links.new(mix_shader_node.outputs[0], output_node.inputs[0])  # Connect Mix Shader to material output

    # Assign the material to the planes
    for plane in [plane_top, plane_bottom, plane_left, plane_right]:
...
def add_lighting() -> None:
    ...
    bpy.data.objects["Area"].scale[0] = 100000
    bpy.data.objects["Area"].scale[1] = 100000
    bpy.data.objects["Area"].scale[2] = 100000
