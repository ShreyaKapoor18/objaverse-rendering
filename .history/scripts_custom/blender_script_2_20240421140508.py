...
def add_background_image(list_env_maps, i):
    ...
    bounding_box_corners = [active_obj.matrix_world @ vector for vector in active_obj.bound_box]

    # Add UV spheres at each bounding box corner and apply environment texture
    for corner in bounding_box_corners:
        add_uv_sphere(corner, radius=0.1)  # Adjust radius as needed
        bpy.context.object.data.materials.append(bpy.data.materials.new(name="HDRI Material"))
        ...
        environment_texture_node.image = bpy.data.images[hdri_path]
        node_tree.links.new(environment_texture_node.outputs['Color'], node_tree.nodes['Principled BSDF'].inputs['Base Color'])
