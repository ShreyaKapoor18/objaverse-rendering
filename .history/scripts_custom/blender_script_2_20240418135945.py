...
def add_background_image(list_env_maps, i):
    ...
    # Remove the default background node
    scene.world.node_tree.nodes.clear()

    # Add Environment Texture node
    env_tex_node = scene.world.node_tree.nodes.new('ShaderNodeTexEnvironment')
    env_tex_node.location = (0, 0)

    # Add Texture Coordinate node
    tex_coord_node = scene.world.node_tree.nodes.new('ShaderNodeTexCoord')
    tex_coord_node.location = (-200, 0)

    # Add Mapping node
    mapping_node = scene.world.node_tree.nodes.new('ShaderNodeMapping')
    mapping_node.location = (-400, 0)

    # Connect Texture Coordinate to Mapping node
    scene.world.node_tree.links.new(tex_coord_node.outputs['Generated'], mapping_node.inputs['Vector'])
    # Add Background node
    bg_node = scene.world.node_tree.nodes.new('ShaderNodeBackground')
    bg_node.location = (200, 0)
    
    scene.world.node_tree.links.new(env_tex_node.outputs['Color'], scene.world.node_tree.nodes['World Output'].inputs['Surface'])


def add_ambient_illumination(intensity):
    # Create or retrieve the world
...
def add_ambient_illumination(list_env_maps, i, intensity):
    # Create or retrieve the world
    world = bpy.context.scene.world
    ...
    env_tex_node.image = bpy.data.images.load(hdr_image_path)  # Load HDR image
...
def save_images(object_file: str) -> None:
    ...
    for i in range(args.num_images):
        ...
        #add_ambient_illumination(intensity=0.5)
        if args.shadows:
        ...
        if args.no_specular:
            ...
            remove_specular()
            # if it set to false\theta = (i / args.num_images) * math.pi * 2
        #theta = np.pi/4
