    ...
    for root, dirs, files in os.walk(directory_path):
        for file in files:
            if file.endswith(('.jpg', '.jpeg', '.png', '.gif')):
                ...
                html_content+=f"""<div class="image-section"><h2>{image_name}</h2>"""
                with open(full_image_path, "rb") as image_file:
                    base64_image = base64.b64encode(image_file.read()).decode("utf-8")
                    html_content += f"""<div class="image-section"><h2>{image_name}</h2><img src="data:image/jpeg;base64,{base64_image}" alt="{full_image_path}"></div>"""
